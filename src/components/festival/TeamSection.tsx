...
export function TeamSection() {
  ...
  useEffect(() => {
    dbQuery<Member>({ table: "team_members", filters: { active: true }, order: { col: "sort_order" } }).then(setMembers);
  }, []);

  return (
    <section className="container-page" style={{ paddingTop: "5rem", paddingBottom: "7rem" }}>
      <div style={{ maxWidth: "42rem" }}>
        <p className="eyebrow">{s.about_board_eyebrow || "Board of Directors"}</p>
        <h2 className="display-lg" style={{ marginTop: "0.5rem" }}>{s.about_board_title || "The volunteers behind NUFF."}</h2>
        <p style={{ marginTop: "1rem", color: "var(--muted-foreground)" }}>{s.about_board_subtitle || "An all-volunteer board guides the festival's vision, finances, and partnerships."}</p>
      </div>
      <div style={{ marginTop: "3rem", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "2rem" }}>
        ...
        {members.length === 0 && <p style={{ color: "var(--muted-foreground)", fontSize: "0.875rem", gridColumn: "1/-1" }}>Board members will appear here once added in Admin.</p>}
      </div>
    </section>
  );
}
