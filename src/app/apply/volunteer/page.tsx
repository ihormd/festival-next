...
export default function VolunteerApplyPage() {
  ...
  if (done) return (
    <div className="container-page" style={{ paddingTop: "5rem", textAlign: "center" }}>
      ...
      <h1 style={{ fontFamily: "Montserrat, sans-serif", fontWeight: 700, fontSize: "1.5rem", marginTop: "1rem" }}>Thanks for volunteering!</h1>
      <p style={{ color: "var(--muted-foreground)", marginTop: "0.5rem" }}>We'll confirm your shifts by email.</p>
    ...
    <div>
      <section style={{ background: "var(--primary)", color: "white", paddingTop: "4rem", paddingBottom: "4rem" }}>
        <div className="container-page">
          <p className="eyebrow" style={{ color: "rgba(255,255,255,0.7)" }}>Volunteer</p>
          <h1 className="display-lg" style={{ color: "white", marginTop: "0.5rem" }}>Volunteer application</h1>
          <p style={{ marginTop: "0.75rem", color: "rgba(255,255,255,0.85)", maxWidth: "38rem", fontSize: "0.95rem" }}>Pick your interests and the shifts that work for you.</p>
        </div>
      </section>
      <section className="container-page" style={{ paddingTop: "3rem", paddingBottom: "4rem", maxWidth: "768px" }}>
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <div><label style={{ display: "block", fontSize: "0.875rem", fontWeight: 600, marginBottom: "0.375rem" }}>Full name</label><input required value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} style={inputStyle} /></div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {shifts.length === 0 && <p style={{ fontSize: "0.875rem", color: "var(--muted-foreground)" }}>Shifts will appear here once added. You can still submit your application.</p>}
              {shifts.map(s => (
                <label key={s.id} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", padding: "0.875rem 1rem", borderRadius: "0.75rem", border: `1px solid ${selectedShifts.has(s.id) ? "var(--primary)" : "var(--border)"}`, cursor: "pointer", transition: "border-color 0.15s" }}>
                  <input type="checkbox" checked={selectedShifts.has(s.id)} onChange={() => toggle(selectedShifts, setSelectedShifts, s.id)} style={{ marginTop: "0.125rem" }} />
                  <div>
          ...
          <div><label style={{ display: "block", fontSize: "0.875rem", fontWeight: 600, marginBottom: "0.375rem" }}>Anything else?</label><textarea rows={3} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} style={{ ...inputStyle, resize: "vertical" }} /></div>
